...
export const listUsers = async ({ status, role, page, limit }) => {
  ...
  const usersResult = await query(
    `SELECT user_id AS "userId", name, email, role, status, registration_no AS "registrationNo", batch_year AS "batchYear", created_at AS "createdAt", updated_at AS "updatedAt"
     FROM users ${where}
...
export const getUserById = async (userId) => {
  const result = await query(
    'SELECT user_id AS "userId", name, email, role, status, registration_no AS "registrationNo", batch_year AS "batchYear", created_at AS "createdAt", updated_at AS "updatedAt" FROM users WHERE user_id = $1',
    [userId]
  );
